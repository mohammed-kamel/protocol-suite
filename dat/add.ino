...
#define HEADER_SIZE 32          //Used for the final message header. 32 bytes = 256 bits
...
// node paremeters Generate the Header
  const byte nodeID_s[SHA256_SIZE]    = { 0x4e, 0x89, 0xd2, 0x11, 0x5f, 0x57, 0xb4, 0xa5, 0xf1, 0x13, 0xe6, 0x25, 0x77, 0xfc, 0xc0, 0x8d, 0x30, 0xea, 0x59, 0x4c, 0xb6, 0x88, 0xa3, 0x9f, 0xfc, 0x69, 0xe6, 0x88, 0x9f, 0x56, 0x9b, 0x8b };
  const byte nodeID_f[SHA256_SIZE]    = { 0xc0, 0x8d, 0x30, 0xea, 0x9b, 0xde, 0x7f, 0x8b, 0x52, 0x8b, 0xcd, 0x57, 0xb4, 0xa5, 0xf1, 0x13, 0xe6, 0x25, 0x77, 0xd2, 0x11, 0xb4, 0xa5, 0xf1, 0x13, 0xe6, 0x25, 0x77, 0xfc, 0xc0, 0x8d, 0xfc };
  byte Hdr[SHA256_SIZE] = { 0x8c, 0xb4, 0xaa, 0x59, 0x4c, 0xb6, 0x88, 0xa3, 0x9f, 0xfc, 0x69, 0xe6, 0x88, 0x9f, 0x56, 0xdd, 0xa4, 0x4c, 0xcc, 0x5f, 0x57, 0xb4, 0xa5, 0xf1, 0x13, 0xe6, 0x25, 0x77, 0xfc, 0x88, 0x77, 0xfc };
...
void setup()
{
  ...
  Serial.begin(115200);
  delay(500);
  Serial.println("DAT Address Distribution Test");
  Serial.println("-----------------------------");
  Serial.println("*****************************");
...
void loop() {
  ...
  Serial.println();
  char nodeAddress[] = "1.2.3.4";
  byte Hdr_new[SHA256_SIZE];
  byte hexHeader[SHA256_SIZE];
  SHA256 hasherR, hasherD;

  //----------------------------------------------------------------- Print the address of the node to the Serial port 
  ...
  //  ################################################
  //  # Prepare the packet for address registration. #
  //  ################################################
  duration = 0;
  timeBegin = micros();

  //----------------------------------------------------------------- Generation of header that will be used during registration
  for(int i = 0; i < SHA256_SIZE; ++i)
    hexHeader[i] = Hdr[i] ^ nodeID_s[i] ^ nodeID_f[i];

  hasherR.doUpdate(hexHeader, SHA256_SIZE);
  hasherR.doFinal(Hdr_new);

  //----------------------------------------------------------------- Encryption of the resource address
  int len = 0;
  bufferSize(nodeAddress, len);
  char encrypted[len];
  encrypt(nodeAddress, encrypted, len);
 ...
 for (byte i=0; i < SHA256_SIZE; i++)
  {
      if (Hdr_new[i]<0x10) { Serial.print('0'); }
      Serial.print(Hdr_new[i], HEX);
  }  
  Serial.println();
  ...
  //  ########################################################
  //  #  Prepare the packet for node address registration.   #
  //  ########################################################
  duration = 0;
  timeBegin = micros();
  
  //----------------------------------------------------------------- Generation of header that will be used for address lookup
  for(int i = 0; i < SHA256_SIZE; ++i)
    hexHeader[i] = Hdr[i] ^ nodeID_s[i] ^ nodeID_f[i];
  hasherD.doUpdate(hexHeader, SHA256_SIZE);
  hasherD.doFinal(Hdr_new);
  

  //----------------------------------------------------------------- Decryption of received encrypted node address
  ...
  Serial.println(duration);
   ...
   // Update the header for the next round
  for (byte i=0; i < SHA256_SIZE; i++)
      Hdr[i] = Hdr_new[i];
